...
object RequestProcessor extends LoggingSupport {
  ...
  def process[F[_]: KafkaSource[*[_], Request]: Concurrent: Logger: Timer: TimeProvider]: F[Unit] = {
    val run =
  ...
  def processEventsChunk[F[_]: Concurrent: Logger](records: Chunk[Request], processSingle: Request => F[Unit]) =
    Logger[F].debug(s"Processing event chunk of size ${records.size}") *> Stream
      .chunk(records)
      .covary[F]
  ...
  def processSingle[F[_]: KafkaSource[*[_], Request]: Concurrent: Logger: TimeProvider](
      request: Request
  ): F[Unit] =
    request.is match {
      case Empty =>
